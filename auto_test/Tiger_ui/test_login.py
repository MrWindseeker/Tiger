...
from common import ExcelData, ExcelConf, Base, TimElem, BasicSev_ui
...
def test_typec(get_log, browser):
    ...
    try:
        ...
        # 用户名输入框
        user_input = BasicSev_ui.show_wait(browser, 'xpath', ui.get('login', 'user_input'), 5, 0.5)
        # 密码输入框
        pwd_input = BasicSev_ui.show_wait(browser, 'xpath', ui.get('login', 'pwd_input'), 5, 0.5)
        user_input.clear()
        user_input.send_keys(user)
        pwd_input.clear()
        pwd_input.send_keys('123456')
        # 登陆按钮
        BasicSev_ui.show_wait(browser, 'xpath', ui.get('login', 'loging_btn'), 5, 0.5).click()
        time.sleep(1)
        # 侧边栏快捷菜单的工时列表按钮
        BasicSev_ui.show_wait(browser, 'xpath', ui.get('home_page', 'Current_Week_Timesheet'), 5, 0.5).click()
        time.sleep(2)
        # 先清空工时表单
        ...
        BasicSev_ui.show_wait(browser, 'xpath', ui.get('timesheet', 'delete_project'), 5, 0.5).click()
        # 添加项目按钮
        BasicSev_ui.show_wait(browser, 'xpath', ui.get('timesheet', 'add_project'), 5, 0.5).click()
        # # 先判断当前工时是否已填写,如果有工时项目了，则点击新增
        # project_code = BasicSev_ui.show_wait(browser, 'xpath', ui.get('timesheet', 'last_tr_engcode')+'/td[5]', 5, 0.5).text
        # if project_code != '':
        #     # 添加项目按钮
        #     BasicSev_ui.show_wait(browser, 'xpath', ui.get('timesheet', 'add_project'), 5, 0.5).click()
        # 点击最后一行的 Look_Up 按钮
        ...
        time.sleep(1)
        activity = BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[1]/div/div/div[2]/div/div/div/div/div[2]/section/div/div[1]/div/div[2]/div[4]/div[2]/div/div[2]/form/div[4]/div/div/div/div/input', 5, 0.5).get_attribute('value')
        if activity is None:
        ...
        element1 = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        if str(class_name1).find('weekendClass') == -1:
            BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[1]/div/div/div[2]/div/div/div/div/div[2]/section/div/div[1]/div/div[2]/div[4]/div[1]/div/div/form/div/div[3]/table/tbody/tr[last()-0]/td[10]/div/div/div/div/input', 5, 0.5).send_keys(8)
        # 获取第二天工时元素属性是否置灰
        element2 = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        if str(class_name2).find('weekendClass') == -1:
            BasicSev_ui.show_wait(browser, 'xpath',
                      ...
                      5, 0.5).send_keys(8)
        # 获取第三天工时元素属性是否置灰
        element3 = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        if str(class_name3).find('weekendClass') == -1:
            BasicSev_ui.show_wait(browser, 'xpath',
                      ...
                      5, 0.5).send_keys(8)
        # 获取第四天工时元素属性是否置灰
        element4 = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        if str(class_name4).find('weekendClass') == -1:
            BasicSev_ui.show_wait(browser, 'xpath',
                      ...
                      5, 0.5).send_keys(8)
        # 获取第五天工时元素属性是否置灰
        element5 = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        if str(class_name5).find('weekendClass') == -1:
            BasicSev_ui.show_wait(browser, 'xpath',
                      ...
                      5, 0.5).send_keys(8)
        # 获取第六天工时元素属性是否置灰
        element6 = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        if str(class_name6).find('weekendClass') == -1:
            BasicSev_ui.show_wait(browser, 'xpath',
                      ...
                      5, 0.5).send_keys(8)
        # 获取第七天工时元素属性是否置灰
        element7 = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        if str(class_name7).find('weekendClass') == -1:
            BasicSev_ui.show_wait(browser, 'xpath',
                      ...
                      5, 0.5).send_keys(8)
        # 提交工时单
        BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[1]/div/div/div[2]/div/div/div/div/div[2]/section/div/div[1]/div/div[1]/div/span[2]/button[4]', 5, 0.5).click()
        time.sleep(1)
        # 确认提交
        BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[1]/div/div/div[2]/div/div/div/div/div[2]/section/div/div[6]/div/div[3]/span/button[1]', 5, 0.5).click()
        # 获取提交后的文本信息
        element_tj = BasicSev_ui.show_wait(browser, 'xpath', ui.get('timesheet', 'submit_message'), 5, 0.5).text
...
def test_typep(get_log, browser):
    user_p = Base.find_dict(user_data, data_key.test_scene, 'P')
    try:
        # 先查询该账号是否有项目
        user = user_p[0][data_key.test_username]
        pwd = user_p[0][data_key.test_password]
        json_login = {'username': user, 'password': pwd, 'captchaVerification': ''}
        login_result = test_admin.login(json_login)
        acs_token = login_result['body']['data']['accessToken']

        data_eng_type = {"engType": "P"}
        type_p_list = test_tim.get_my_eng(acs_token, data_eng_type)
        eng_p_list = type_p_list['body']['data']
        get_log.info(user + '用户的项目：' + str(eng_p_list))
        # 先判断该用户typec的项目数量
        # 如果没有项目，则通过接口新增一个
        engName = ''
        if len(eng_p_list) <= 0:
        ...
        get_log.info('engagement元素选择的项目名为：' + engName)
        browser.get(r"https://bcp-preuat.ey.com.cn/login?redirect=%2Ftim%2F")
        user_input = BasicSev_ui.show_wait(browser, 'xpath',
                         '/html/body/div/div/div[1]/div[2]/div/div[2]/form/div[2]/div[1]/div/div/input', 5, 0.5)
        pwd_input = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        pwd_input.send_keys('123456')
        # 登陆按钮
        BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div/div/div[1]/div[2]/div/div[2]/form/div[3]/div/button', 5,
                  0.5).click()
        # 侧边栏快捷菜单的工时列表按钮
        BasicSev_ui.show_wait(browser, 'xpath',
        ...
        time.sleep(2)
        # 先判断当前工时是否已填写,如果有工时项目了，则点击新增
        project_code = BasicSev_ui.show_wait(browser, 'xpath',
        ...
        BasicSev_ui.show_wait(browser, 'xpath',
                  ...
                  5, 0.5).click()
        # 选择Type框内的元素
        BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[3]/div[1]/div[1]/ul/li[2]', 5, 0.5).click()
        ...
        # 选择Engagement框内的元素
        BasicSev_ui.show_wait(browser, 'xpath', f"//*[text()='{engName}']", 5, 0.5).click()
        # BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[4]/div[1]/div[1]/ul/li[1]', 5, 0.5).click()
        time.sleep(1)
        activity = BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[1]/div/div/div[2]/div/div/div/div/div[2]/section/div/div[1]/div/div[2]/div[4]/div[2]/div/div[2]/form/div[4]/div/div/div/div/input', 5, 0.5).get_attribute('value')
        if activity is None:
            get_log.error('Activity为空')
        ...
        time.sleep(2)
        # 获取提交成功的文本信息
        element_tj = BasicSev_ui.show_wait(browser, 'xpath', '/html/body/div[4]/p', 5, 0.5).text
